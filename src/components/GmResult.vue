          ...
          <div class="row font-12">
            <div class="filter-left col-md-6">
              ...
              <div class="row-div row">
                <span class="row-name col-md-2">过滤条件: </span>
              </div>
              ...
              <div class="row-div row">
                <span class="row-name col-md-2">MITIPACT</span>
                <div class="row-content col-md-10">
                  <span class="condition" @click="conClick" data-name="mitipact" data-value="true">已报道</span>
                  <span class="condition" @click="conClick" data-name="mitipact" data-value="false">未报道</span>
                  <span class="condition on con-default" @click="conClick">不筛选</span>
                </div>
              </div>

              <div class="row-div row">
                <span class="row-name col-md-2">MITOMAP</span>
                <div class="row-content col-md-10">
                  <span class="condition" @click="conClick" data-name="mitomap" data-value="true">已报道</span>
                  <span class="condition" @click="conClick" data-name="mitomap" data-value="false">未报道</span>
                  <span class="condition on con-default" @click="conClick">不筛选</span>
                </div>
              </div>
            </div>
          ...
          <div class="re">
            <table class="table table-gene task-result-table myTable" id="aberrance">
              ...
              <tr v-for="data in detailAllData" @click="addBc">
                <td>
                  <i v-if="data.level==0" :data-code="data.code" :data-id='data.id' class="fa fa-font-awesome text-1 po"
                     title="查看详情" @click="showDetail"></i>  <!--最高级-->
                  <i v-else-if="data.level==1" :data-code="data.code" :data-id='data.id'
                     class="fa fa-font-awesome text-2 po"
                     title="查看详情" @click="showDetail"></i> <!--第二级-->
                  <i v-else :data-code="data.code" :data-id='data.id' class="fa fa-font-awesome po text-2" title="查看详情"
                     @click="showDetail"></i><!--第三级-->
                  <!--<span v-if="data.localsnv">{{data.localsnv.name}}</span>-->

                  <a class="po" v-if="data.localsnv" @click="showLocus0('MT:'+data.localsnv.start+':'+data.localsnv.end+':'+data.localsnv.ref+':'+data.localsnv.alt,0)">
                    {{data.localsnv.name}}
                  </a>
                ...
                <td> -
                  ...
                  <!--{{data.annotations.grandfreq | filterData}}-->
                  <!--</span>-->
                </td>
                <td v-if="data.annotations">
                  <!--{{data.annotations.mitimpact}}-->
                  <div v-for="single in data.annotations.mitimpact.split(';')">
                    {{single}}
                  </div>
                </td>
                <td v-if="data.annotations">
                  <!--{{data.annotations.mitomap}}-->
            ...
            <div class="modal-dialog modal-dialog modal-lg" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body" id="id_modal">
                  <div class="row">
                    <div class="col-md-4">
                      <a v-if="moduleData.localsnv" class="analyze-mutate po" :href="dbHtml+'#/mutateDetail?query='+moduleData.localsnv.chrom + ':'
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-4">
                      ...
                      人群频率：<span v-if="moduleData.annotations">
                      {{moduleData.annotations.mtdb | filterData}}
                    </span>
                    </div>
                  </div>
                  ...
                  <div class="row">
                    <div class="col-md-4">
                      本地人群频率：-
                      <!--<span v-if="moduleData.annotations">{{moduleData.annotations.grandfreq | filterData}}</span>-->
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-md-11">
        ...
        <div role="tabpanel" class="tab-pane" id="settings">
          <div class="title-content row">
            <div class="col-md-offset-4 col-md-3">
              <textarea class="form-control" id="countInput" placeholder="输入gene名称，逗号或换行分隔"></textarea>
            </div>
            <div class="col-md-1">
            </div>
          </div>
          <button id="search-btn" class="btn btn-color find-cover" @click="geneCoverEnter">查&nbsp;&nbsp;询</button>
...
//          qObj.gender = resp.data.gender === '女' ? 'Female' : 'Male';
      ...
      geneCoverEnter: function () {
        ...
        this.$axios({
          url: 'application/grandmito/' + this.ID + '/cov/',
          method: 'post',
...
////                  if (count === value.annotations.geneUrl.length) {
////                    console.log(value);
////                    _vue.detailAllData.push(value); //填充最外层数据
////                    _vue.detailAllData = _vue.detailAllData.sort(function (a, b) {
////                      return a.id - b.id
////                    });
////                  }
...
//        const id = _comment.data('id');
        ...
        this.$axios({
          ...
          method: 'patch',
          data: {
            'status': $("#select").find("option:selected").val(),
            'comment': ajaxValue
          }
        }).then(function (resp) {
          $.each(_vue.detailAllData, function (i, data) {
            if (data.id === eid) {
              data.status = _vue.moduleData.status;
